...
/** Resolve @filepath convention: if value starts with "@", read file content instead */
function resolveArgValue(value: string): string {
  if (value.startsWith("@") && value.length > 1) {
    const filePath = value.slice(1);
    try {
      return fs.readFileSync(filePath, "utf-8").trim();
    } catch {
      // fall through — treat as literal value
    }
  }
  return value;
}
...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--type" && argv[i + 1]) {
      ...
      args.title = resolveArgValue(argv[++i]!);
    } else if (arg === "--author" && argv[i + 1]) {
      args.author = argv[++i];
    } else if (arg === "--summary" && argv[i + 1]) {
      args.summary = resolveArgValue(argv[++i]!);
